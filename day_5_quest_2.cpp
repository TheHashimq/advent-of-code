...
get_un_ordered_pages(std::vector<std::vector<int>> &pagingList,
...
std::vector<int> make_it_ordered(std::vector<int> &unOrderedPages,
                                 std::vector<std::array<int, 2>> orderedRules);
...
int main(int argc, char **argv) {
  ...
  // Process
  std::vector<std::vector<int>> unOrderedPages =
      get_un_ordered_pages(pagingList, pagingOrder);
  for (auto &v : unOrderedPages) {
    v = make_it_ordered(v, get_order_rules(v, pagingOrder));
  }

  int middle_sum = get_middle_page_sum(unOrderedPages);
  // Output
  std::cout << "Sum of Middle Pages of Un ordered pages = " << middle_sum
            << std::endl;
  return 0;
}
...
std::vector<int> make_it_ordered(std::vector<int> &unOrderedPages,
  ...
  for (const auto &rule : orderedRules) {
    ...
    auto posX = std::find(unOrderedPages.begin(), unOrderedPages.end(), x);
    auto posY = std::find(unOrderedPages.begin(), unOrderedPages.end(), y);
    if (posX != unOrderedPages.end() && posY != unOrderedPages.end() &&
        posX > posY) {
      std::iter_swap(posX, posY);
    }
  }
  while (!is_ordered(unOrderedPages, orderedRules)) {
    make_it_ordered(unOrderedPages, orderedRules);
  }
  return unOrderedPages;
}
